...
class Grafo:
    ...
    def dijkstra(self, origem):
        custo_e_de_onde_vem = [[-1, 0] for i in range(self.vertices)]
        custo_e_de_onde_vem[origem - 1] = [0, origem]
        h = HeapMin()
        h.ad_custo(0, origem)
        while h.tamanho() > 0:
            dist, v = h.rem_custo()
            for i in range(self.vertices):
                if self.grafo[v-1][i] != 0:     #self.grafo = matriz de distância
                    if custo_e_de_onde_vem[i][0] == -1 or custo_e_de_onde_vem[i][0] > dist + self.grafo[v-1][i]:
                        custo_e_de_onde_vem[i] = [dist + self.grafo[v-1][i], v]
                        h.ad_custo(dist + self.grafo[v-1][i], i+1)
        return custo_e_de_onde_vem
